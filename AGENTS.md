...
- Official certification pages may be linked as objectives, but explanations,
  questions, and labs must be original.
- Prefer small, validated changes. Run `uv run ruff check .`,
  `uv run ruff format --check .`, and `uv run pytest -q` for code changes.
